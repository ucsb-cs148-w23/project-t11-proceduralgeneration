...
class SocketGenerator:
    ...
    def __init__(self, mesh_list=[], mesh_size=2):
        self._protos = {
            f"proto_{i}": {"mesh": mesh, "rotation": 0, "sockets": {}}
        ...
        self._bounds = {
            d: (0.5 if d[0] == "p" else -0.5) * mesh_size
    ...
    def add_mesh(self, mesh_file):
        ...
        for d in SocketGenerator._DIRECTIONS:
            ...
            self._protos[proto_name]["sockets"][d] = self._process_face(
                border_vertices[d], mode=mode
            )
    ...
    def _border_is_symmetrical_y(border_vertices):
        ...
        for vertex in border_vertices:
            if not SocketGenerator._contains_close(vertex, reflected):
                return False
    ...
    def _vertices_equiv(vertices1, vertices2):
        for v in vertices1:
            if not SocketGenerator._contains_close(v, vertices2):
                return False

        for v in vertices2:
            if not SocketGenerator._contains_close(v, vertices1):
                return False
    ...
    def _normalize_face(vertices, direction):
        if direction == "px":
            vertices = [[-y, z] for x, y, z in vertices]
        if direction == "ny":
            vertices = [[-x, z] for x, y, z in vertices]
        if direction == "nx":
            vertices = [[y, z] for x, y, z in vertices]
        if direction == "py":
            vertices = [[x, z] for x, y, z in vertices]
    ...
    def _get_border_vertices_dict(self, mesh):
        ...
        for vertex in mesh.vertices:
            ...
            if math.isclose(vertex[0], self._bounds["px"]):
                border_vertices["px"].append(vertex)
            if math.isclose(vertex[2], self._bounds["pz"]):
                border_vertices["pz"].append(vertex)
            if math.isclose(vertex[1], self._bounds["py"]):
                border_vertices["py"].append(vertex)
            if math.isclose(vertex[0], self._bounds["nx"]):
                border_vertices["nx"].append(vertex)
            if math.isclose(vertex[1], self._bounds["ny"]):
                border_vertices["ny"].append(vertex)
            if math.isclose(vertex[2], self._bounds["nz"]):
                border_vertices["nz"].append(vertex)

        border_vertices = {
            d: SocketGenerator._normalize_face(border_vertices[d], d)
            for d in SocketGenerator._DIRECTIONS
        }
    ...
    def _face_find(self, vertices, mode="lateral"):
        ...
        for face_vertices in seen:
            if SocketGenerator._vertices_equiv(vertices, face_vertices):
                return sockets[SocketGenerator._serialize(face_vertices)]
    ...
    def _process_prototypes(self):
        for key, proto in self._protos.items():
            ...
            for d in SocketGenerator._DIRECTIONS:
                mode = "lateral" if d[1] != "z" else "vertical"
                self._protos[key]["sockets"][d] = self._process_face(
                    border_vertices[d], mode=mode
                )
